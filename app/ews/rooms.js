    ...
    var promise = new Promise(function(resolve, reject) {
      ...
      roomAddresses.forEach(function(room, index, array) {
        var calendarFolderId = new ews.FolderId(
          ...
          new ews.Mailbox(room.Email)
        );
        var view = new ews.CalendarView(
          ews.DateTime.Now,
          new ews.DateTime(
            ews.DateTime.Now.TotalMilliSeconds +
              ews.TimeSpan.FromHours(240).asMilliseconds()
          ),
          4
        );
        exch.FindAppointments(calendarFolderId, view).then(
          ...
          error => {
            ...
            // callback(error, null);
            fillRoomData(context, room, undefined, {
